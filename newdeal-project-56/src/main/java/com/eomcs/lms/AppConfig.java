...
//위와 같이 IoC 컨테이너를 생성할 때 AppConfig를 직접 지정한다면 다음 애노테이션은 필요없다
//그러나 XML 설정을 사용할 경우 이 애노테이션 사용해야 
//IoC 컨테이너가 이 클래스가 스프링 설정 정보를 다루는 클래스임을 인식할 수 있다.
@Configuration
...
//인스턴스를 자동으로 생성해줌
//-> XML 설정에 추가되어 있다면 다음은 제거
/*@ComponentScan("com.eomcs.lms")*/

//Spring IoC 컨테이너에게 프로퍼티 파일을 읽으라고 로딩할 것을 명령
@PropertySource("classpath:/com/eomcs/lms/conf/jdbc.properties")

//MyBatis에 DAO 구현체를 자동생성하는 도우미를 추가
//-> DAO 인터페이스가 들어있는 패키지를 지정
//-> 자동 생성된 DAO 구현체가 SQL을 찾을 때 ㅇ인터페이스의 전체 이름으로 찾는다
//-> 따라서 sql 매퍼 파일의 namespace이름은 인터페이스의 전체이름과 같아야 한다
//인터페이스의 메소드명, 메소드 파라미터와 리턴타입은 SQL 매퍼와 같아야 한다
...
public class AppConfig {
 ...
 //Spring WebMVC에 기본으로 설정되어 있는 ViewResolver를 InternalResourceViewResolver로 교체
  //페이지 컨트롤러가 리턴하는url 앞뒤에 설정된 경로를 붙인다.
  //-> xml에서 ViewResolver 객체를 준비했다면 다음 메소드는 제거한다
  /*@Bean
