...
- 같은 데이터를 중복 작성하지 않음으로 데이터 동기화 시 문제를 안 만들기 위해.
   아니면 강사 이메일 바뀔 때마다 그 강사의 이메일을 전부 수정해줘야 한다.★
...
- ★수강생 테이블에서, 우편번호 컬럼이 바뀌면 기본주소 컬럼도 바뀌어야 함
...
- 포함 관계★
  - 여러 테이블에서 동시에 포함할 수 있는 관계이다.
    (회원정보 테이블은 강사, 수강생, 매니저의 정보를 모두 가질 수 있다)
- 배타적 관계
  - 여러 테이블 중에서 오직 한 개의 테이블만 포함할 수 있는 관계다.  
- exERD에서는 포함, 배타적을 표현할 수 없고 그냥 주석으로 설명하는 수밖엔 없음
